...
import { batchFuzzyMatch, FuzzyMatchResult, AdministrativeOption } from './fuzzy-matching';
import type { CurrentGeocodingResponse } from '../types/administrative-sync-analysis';
import { administrativeService, type Province, type Regency, type District } from '../../services/api-client';
...
export function processGeocodingResponse(
  ...
  const validation = validateHierarchicalRelationships(enhancedResponse, administrativeData);
...
function validateHierarchicalRelationships(
  ...
  if (response.administrative.province.code && response.administrative.regency.code) {
    const regency = administrativeData.regencies.find(r => r.code === response.administrative.regency.code);
    if (regency && 'province_code' in regency && regency.province_code !== response.administrative.province.code) {
      issues.push(`Regency ${response.administrative.regency.name} does not belong to province ${response.administrative.province.name}`);
    }
  }

  // Check if district belongs to regency
  if (response.administrative.regency.code && response.administrative.district.code) {
    const district = administrativeData.districts.find(d => d.code === response.administrative.district.code);
    if (district && 'regency_code' in district && district.regency_code !== response.administrative.regency.code) {
      issues.push(`District ${response.administrative.district.name} does not belong to regency ${response.administrative.regency.name}`);
    }
  }
...
export function getAdministrativeSyncStatus(
  ...
  const hasAnyMatch = administrative.province.code || administrative.regency.code || administrative.district.code;
...
export function applyEnhancedGeocodingToForm(
  ...
  // Apply province
  if (enhancedResponse.administrative.province.code && enhancedResponse.administrative.province.name) {
    form.setValue('province_code', enhancedResponse.administrative.province.code);
  ...
  // Apply regency
  if (enhancedResponse.administrative.regency.code && enhancedResponse.administrative.regency.name) {
  ...
  // Apply district
  if (enhancedResponse.administrative.district.code && enhancedResponse.administrative.district.name) {
    form.setValue('district_code', enhancedResponse.administrative.district.code);
 ...
 */
export function getMatchTypeDescription(matchType: 'exact' | 'synonym' | 'fuzzy' | 'none'): {
  description: string;
  icon: string;
} {
  switch (matchType) {
...
export async function processNominatimAddressWithAPI(
  ...
  const results = {
    province: { code: null as string | null, name: null as string | null, confidence: 0, matchType: 'none' as 'exact' | 'synonym' | 'fuzzy' | 'none' },
    regency: { code: null as string | null, name: null as string | null, confidence: 0, matchType: 'none' as 'exact' | 'synonym' | 'fuzzy' | 'none' },
    district: { code: null as string | null, name: null as string | null, confidence: 0, matchType: 'none' as 'exact' | 'synonym' | 'fuzzy' | 'none' }
  };

  try {
    // Step 1: Search for province using API
    if (geocodingResponse.province) {
      const provinceMatch = await administrativeService.searchProvinces(geocodingResponse.province);
      if (provinceMatch) {
        results.province = {
          code: provinceMatch.code,
          name: provinceMatch.name,
          confidence: calculateMatchConfidence(geocodingResponse.province, provinceMatch.name),
          matchType: getMatchType(geocodingResponse.province, provinceMatch.name)
        };
        ...
        if (geocodingResponse.city && results.province.code) {
          ...
          if (regencyMatch) {
            results.regency = {
              ...
              confidence: calculateMatchConfidence(geocodingResponse.city, regencyMatch.name),
              matchType: getMatchType(geocodingResponse.city, regencyMatch.name)
            };
            ...
            if (geocodingResponse.district && results.regency.code) {
              ...
              if (districtMatch) {
                results.district = {
                  ...
                  confidence: calculateMatchConfidence(geocodingResponse.district, districtMatch.name),
                  matchType: getMatchType(geocodingResponse.district, districtMatch.name)
                };
              }
            }
          }
 ...
 */
function calculateMatchConfidence(searchTerm: string, resultName: string): number {
 ...
 */
function getMatchType(searchTerm: string, resultName: string): 'exact' | 'synonym' | 'fuzzy' | 'none' {
  ...
  const synonymMap: Record<string, string[]> = {
    ...
    'jakarta': ['dki jakarta', 'jakarta pusat', 'jakarta selatan', 'jakarta barat', 'jakarta utara', 'jakarta timur'],
  ...
  if (synonymMap[searchKey]?.some(synonym => synonym.includes(resultKey)) ||
 ...
 */
function validateAPIBasedHierarchy(
  response: EnhancedGeocodingResponse
): { isValid: boolean; issues: string[] } {
  const issues: string[] = [];
  ...
  if (response.administrative.regency.code && !response.administrative.province.code) {
    issues.push('Regency found without province - this should not happen with API-based search');
  }
  
  if (response.administrative.district.code && !response.administrative.regency.code) {
    issues.push('District found without regency - this should not happen with API-based search');
  }
