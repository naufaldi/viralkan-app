...
export function ShareDialog({
  ...
  const { isGenerating, generateAICaption, trackShare, shareToPlatform } = useSharing({
    onShareSuccess,
    onError: (error) => {
  ...
  const handleShare = async () => {
    ...
    if (!success) {
      toast.error(`Gagal membuka platform. Silakan salin caption dan link secara manual.`);
    }
              ...
              <div className="flex items-center gap-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-blue-700">
                    Model: {modelUsed === 'template-fallback' ? 'Template (AI gagal)' : modelUsed}
                  </p>
                </div>
