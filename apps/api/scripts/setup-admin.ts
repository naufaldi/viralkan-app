...
async function confirmAdminSetup(adminEmails: string[]): Promise<boolean> {
  ...
  console.log("==================================");
  console.log(
    "You are about to grant admin privileges to the following users:",
  );
...
async function setupAdmins() {
  ...
  for (const email of adminEmails) {
    ...
    if (!userExists) {
      ...
      console.log(
        `  💡 User must first login through the application to be created`,
      );
      results.push({
        email,
        success: false,
