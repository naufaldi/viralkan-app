...
const responseCache = new Map<string, { data: AICaptionResponse; timestamp: number; ttl: number }>();
...
const getConfig = () => {
  if (!aiConfig) {
    try {
      ...
      console.warn('AI configuration failed:', error instanceof Error ? error.message : 'Unknown error');
      // Fallback config
      aiConfig = {
        apiKey: '',
...
const getPlatformConfig = (platform: Platform) => {
  const configs = {
    twitter: { maxLength: 280, hashtagLimit: 3, urlHandling: 'included' as const },
    facebook: { maxLength: 2000, hashtagLimit: 5, urlHandling: 'separate' as const },
    whatsapp: { maxLength: 1000, hashtagLimit: 3, urlHandling: 'included' as const },
    threads: { maxLength: 2200, hashtagLimit: 10, urlHandling: 'separate' as const },
    telegram: { maxLength: 4096, hashtagLimit: 5, urlHandling: 'included' as const },
  };

  const config = configs[platform];
  if (!config) {
    console.warn(`Unknown platform '${platform}', falling back to default config`);
    return { maxLength: 1000, hashtagLimit: 3, urlHandling: 'included' as const };
  }
...
const generateSystemPrompt = (tone: CaptionTone, platform: Platform): string => {
  const platformConfig = getPlatformConfig(platform);
  
  // Get tone description from the single source of truth
  const toneConfig = AVAILABLE_TONES.find(t => t.value === tone);
  const toneDescription = toneConfig?.description || 'Generate appropriate Indonesian caption';
...
// AI response parsing
const parseAIResponse = (content: string): AppResult<{ caption: string; hashtags: string[] }> => {
  try {
...
export const generateAICaption = async (request: AICaptionRequest): Promise<AppResult<AICaptionResponse>> => {
  const MAX_RETRIES = 3;
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (request.usePaidModel === false) {
        ...
        modelsToTry = [{ model: config.modelFree, isPaid: false, timeout: 30000 }];
      } else {
      ...
      for (const { model, isPaid, timeout } of modelsToTry) {
        try {
          ...
          const systemPrompt = generateSystemPrompt(request.tone, request.platform);
          const userPrompt = generateUserPrompt(request.reportData, request.tone, request.platform);
          ...
          console.log(`Generated AI response for attempt ${attempt} in ${Date.now() - startTime}ms`);
          ...
          if (isRetryableError && !isPaid) {
            console.warn(`Free model failed with retryable error: ${lastError.message}. Retrying with paid model...`);
            continue; // Try next model (paid)
          } else if (isRetryableError && isPaid) {
            console.error(`Both free and paid models failed with retryable errors. Last error: ${lastError.message}`);
            break; // Both models failed, don't retry further
          } else {
            // Non-retryable error, don't retry
            console.error(`AI service error (non-retryable): ${lastError.message}`);
            break;
          }
        }
      }
      ...
      console.error(`All AI models failed on attempt ${attempt}:`, lastError?.message);
      
      // If this is not the last attempt, wait and retry
      if (attempt < MAX_RETRIES) {
        console.log(`Retrying AI service call... (attempt ${attempt + 1}/${MAX_RETRIES})`);
      ...
      console.error(`AI caption generation failed on attempt ${attempt}:`, error);
      
      // If this is not the last attempt, wait and retry
      if (attempt < MAX_RETRIES) {
        console.log(`Retrying AI service call after error... (attempt ${attempt + 1}/${MAX_RETRIES})`);
        await new Promise(resolve => setTimeout(resolve, RETRY_DELAY * attempt));
        continue;
      }
...
export const checkAIHealth = async (): Promise<AppResult<{ status: string; model: string }>> => {
  try {
    const config = getConfig();
    const client = getClient();
...
export const getAvailableModels = (): { 
  ...
  return {
    ...
    paid: {
      ...
      description: 'DeepSeek V3 with optimized provider fallback (Targon→DeepInfra→Lambda) for best performance.'
    }
  };
};
