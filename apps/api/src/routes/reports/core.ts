...
export const validateReportData = (data: CreateReportInput): AppResult<CreateReportInput> => {
  // Business rule: Coordinates must be both provided or both omitted
  if ((data.lat !== undefined && data.lon === undefined) || 
      (data.lat === undefined && data.lon !== undefined)) {
    return createError('Both latitude and longitude must be provided together', 400);
  }

  // Business rule: Validate coordinate ranges for Indonesia
  if (data.lat !== undefined && data.lon !== undefined) {
    if (data.lat < -11 || data.lat > 6) {
      return createError('Latitude must be within Indonesia bounds (-11 to 6)', 400);
    }
    if (data.lon < 95 || data.lon > 141) {
      return createError('Longitude must be within Indonesia bounds (95 to 141)', 400);
    }
  }
...
export const sanitizeReportData = (data: CreateReportInput): CreateReportInput => {
  return {
    ...data,
...
export const calculateReportPriority = (report: ReportWithUser): 'high' | 'medium' | 'low' => {
...
export const canUserEditReport = (report: ReportWithUser, userId: number): boolean => {
  // Business rule: Only the report creator can edit their report
  return report.user_id === userId;
};

export const canUserDeleteReport = (report: ReportWithUser, userId: number): boolean => {
  // Business rule: Only the report creator can delete their report
  return report.user_id === userId;
};

export const isReportStale = (report: ReportWithUser, daysThreshold: number = 30): boolean => {
...
export const formatReportForDisplay = (report: ReportWithUser): ReportWithUser => {
...
export const validatePaginationParams = (page: number, limit: number): AppResult<{ page: number; limit: number }> => {
...
export const enrichReportData = (report: ReportWithUser): ReportWithUser & { 
...
export const sortReportsByPriority = (reports: ReportWithUser[]): ReportWithUser[] => {
