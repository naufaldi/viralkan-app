...
reportsRouter.openapi(updateReportRoute, async (c) => {
  try {
    ...
    return c.json({
      error: {
        code: result.statusCode === 403 ? 'FORBIDDEN' : result.statusCode === 404 ? 'NOT_FOUND' : 'UPDATE_ERROR',
        message: result.error,
        timestamp: new Date().toISOString(),
      },
...
reportsRouter.openapi(deleteReportRoute, async (c) => {
  try {
    ...
    return c.json({
      error: {
        code: result.statusCode === 403 ? 'FORBIDDEN' : result.statusCode === 404 ? 'NOT_FOUND' : 'DELETE_ERROR',
        message: result.error,
        timestamp: new Date().toISOString(),
      },
    }, result.statusCode as 400 | 401 | 403 | 404 | 500)
...
reportsRouter.openapi(validateOwnershipRoute, async (c) => {
  try {
    ...
    return c.json({
      error: {
        code: result.statusCode === 403 ? 'FORBIDDEN' : result.statusCode === 404 ? 'NOT_FOUND' : 'VALIDATION_ERROR',
        message: result.error,
        timestamp: new Date().toISOString(),
      },
    }, result.statusCode as 400 | 401 | 403 | 404 | 500)
