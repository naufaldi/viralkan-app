...
export const firebaseAuthMiddleware = async (c: AuthContext, next: Next) => {
  try {
    console.log(
      `[DEBUG] firebaseAuthMiddleware - Starting authentication check`,
    );

    const authHeader = c.req.header("Authorization");
    console.log(
      `[DEBUG] firebaseAuthMiddleware - Auth header: ${authHeader ? "Present" : "Missing"}`,
    );

    if (!authHeader || !authHeader.startsWith("Bearer ")) {
      console.log(
        `[DEBUG] firebaseAuthMiddleware - Invalid auth header format`,
      );
    ...
    console.log(
      `[DEBUG] firebaseAuthMiddleware - Token extracted: ${idToken ? "Present" : "Missing"}`,
    );
    ...
    console.log(
      `[DEBUG] firebaseAuthMiddleware - Calling shell.verifyTokenAndGetUser`,
    );
    ...
    if (!result.success) {
      console.log(
        `[DEBUG] firebaseAuthMiddleware - Authentication failed: ${result.error}`,
      );
      const errorResponse: ErrorResponse = {
        error: result.error,
    ...
    console.log(
      `[DEBUG] firebaseAuthMiddleware - Setting user_id in context: ${result.data.user_id}`,
    );
    c.set("user_id", result.data.user_id);

    console.log(
      `[DEBUG] firebaseAuthMiddleware - Authentication successful, calling next()`,
    );
    await next();
  } catch (error) {
...
export const requireRole = (requiredRole: string) => {
  return async (c: AuthContext, next: Next) => {
    try {
      ...
      console.log(
        `[DEBUG] requireRole middleware - userId: ${userId}, requiredRole: ${requiredRole}`,
      );

      if (!userId) {
        console.log(
          `[DEBUG] requireRole middleware - No userId found in context`,
        );
        const errorResponse: ErrorResponse = {
      ...
      if (result.data.role !== requiredRole) {
        console.log(
          `[DEBUG] requireRole middleware - Role mismatch. Expected: ${requiredRole}, Got: ${result.data.role}`,
        );
        const errorResponse: ErrorResponse = {
      ...
      console.log(
        `[DEBUG] requireRole middleware - Authorization successful for user ${userId} with role ${result.data.role}`,
      );
      await next();
    } catch (error) {
