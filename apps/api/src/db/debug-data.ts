...
const debugData = async () => {
  ...
  try {
    ...
    console.log("\n📋 Users table:");
    const users =
      await sql`SELECT id, firebase_uid, email, name FROM users LIMIT 5`;
    console.log(users);

    // Check reports table with status
    console.log("\n📋 Reports table with status:");
    const reports =
      await sql`SELECT id, user_id, category, street_name, status, created_at FROM reports LIMIT 5`;
    console.log(reports);
    ...
    const user =
      await sql`SELECT id, firebase_uid, email, name FROM users WHERE firebase_uid = ${firebaseUid}`;
    ...
    if (user.length > 0) {
      ...
      const userReports =
        await sql`SELECT id, user_id, category, street_name, status, created_at FROM reports WHERE user_id = ${userId}`;
      ...
      const verifiedReports =
        await sql`SELECT id, user_id, category, street_name, status, created_at FROM reports WHERE user_id = ${userId} AND status = 'verified'`;
