...
export const env = {
  ...
  FIREBASE_SERVICE_ACCOUNT_JSON: getRequiredEnv('FIREBASE_SERVICE_ACCOUNT_JSON'),
...
export const validateEnv = (): void => {
  try {
    ...
    if (env.NODE_ENV === 'development') {
      ...
      if (!env.GOOGLE_CLIENT_SECRET) optionalWarnings.push('GOOGLE_CLIENT_SECRET')
      ...
      if (optionalWarnings.length > 0) {
        console.warn(`⚠️ Optional environment variables not set (features may be limited): ${optionalWarnings.join(', ')}`)
      }
    }
  } catch (error) {
